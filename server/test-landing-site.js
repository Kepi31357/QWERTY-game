...
assert(/background-color:\s*#d9c7f2\s*!important/.test(css), 'forced lavender card background');
assert(landing.indexOf('shot-board') >= 0, 'screenshot board chrome wrapper');
assert(/#screens \.shot-board[\s\S]*?padding:\s*16px\s*!important/.test(css) || /\.shot-board\s*\{[\s\S]*?padding:\s*16px\s*!important/.test(css), 'forced shot-board padding 16px');
